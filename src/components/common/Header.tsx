...
export const Header: React.FC = () => {
  ...
  const [selectedRegionFilter, setSelectedRegionFilter] = useState<'All' | 'North India' | 'West India' | 'South India' | 'East India' | 'Central India' | 'North-East'>('All');
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="relative" ref={locRef}>
              ...
              {showLocationDropdown && (
                <div className="absolute right-0 mt-2 w-80 sm:w-96 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                  <div className="pb-2 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <div>
                      <span className="text-xs font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5">
                        ...
                        India CPCB Sensor Stations
                      </span>
                      <p className="text-[10px] text-slate-400">
                        Nationwide real-time ambient monitoring nodes
                      </p>
                    </div>
                    <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300 font-semibold border border-emerald-200 dark:border-emerald-800">
                  ...
                  <div className="mt-2 relative">
                    <input
                      type="text"
                      placeholder="Search city, station, state (e.g. Pune, Kanpur, Delhi)..."
                      value={locationSearchQuery}
                      onChange={(e) => setLocationSearchQuery(e.target.value)}
                      className="w-full text-xs py-1.5 pl-3 pr-8 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                    />
                    {locationSearchQuery && (
                      <button
                  ...
                  {/* Region Filter Chips */}
                  <div className="flex items-center gap-1 overflow-x-auto py-2 no-scrollbar text-[10px]">
                    {(['All', 'North India', 'West India', 'South India', 'East India', 'Central India', 'North-East'] as const).map(
                      (region) => (
                        ...
                        >
                          {region === 'All' ? 'All India' : region.replace(' India', '')}
                        </button>
                      )
                    )}
                  </div>
                  ...
                  <div className="max-h-64 overflow-y-auto py-1 divide-y divide-slate-100 dark:divide-slate-800/60">
                    {/* If user typed a query, offer to resolve ANY location in India */}
                    {locationSearchQuery.trim().length > 1 && (
                      <button
                        onClick={() => {
                          searchAndSelectIndianLocation(locationSearchQuery);
                          setShowLocationDropdown(false);
                          setLocationSearchQuery('');
                        }}
                        className="w-full mb-1 text-left px-3 py-2 rounded-xl bg-gradient-to-r from-emerald-500/10 to-teal-500/10 hover:from-emerald-500/20 hover:to-teal-500/20 border border-emerald-500/30 flex items-center justify-between transition-colors"
                      >
                        <div>
                          <p className="text-xs font-bold text-emerald-800 dark:text-emerald-300">
                            + Lookup "{locationSearchQuery}" in India
                          </p>
                          <p className="text-[10px] text-emerald-700/70 dark:text-emerald-400">
                            Generate CPCB telemetry for this Indian city/district
                          </p>
                        </div>
                    ...
                    {locations
                      .filter((loc) => {
                        ...
                        const matchesSearch =
                          ...
                          (loc.cpcbStationCode && loc.cpcbStationCode.toLowerCase().includes(q)) ||
                          loc.district.toLowerCase().includes(q);

                        const matchesRegion =
                          selectedRegionFilter === 'All' || loc.region === selectedRegionFilter;
                          ...
                          >
                            <div className="pr-2 min-w-0">
                              <div className="flex items-center gap-1.5">
                                ...
                                {loc.cpcbStationCode && (
                                  <span className="text-[9px] font-mono px-1 py-0.2 rounded bg-slate-100 dark:bg-slate-800 text-slate-500 shrink-0">
                                    {loc.cpcbStationCode}
                                  </span>
                                )}
                              </div>
                              <p className="text-[10px] text-slate-400 dark:text-slate-500 truncate">
                                {loc.city}, {loc.state} • {loc.region || 'India'}
                              </p>
                            </div>
